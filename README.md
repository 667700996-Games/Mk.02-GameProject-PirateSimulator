...
- 난파선 잔해를 제외한 핵심 11종, 산업 9종, 주거·복지·군수·행정 9종은 2단계·3단계 전용 본체 아틀라스를 사용한다. 핵심 시설은 천막·부두·포대 규모가, 산업 시설은 채굴 설비·경작 단수·용광로·작업 베이·가공 및 저장 설비가, 사회 시설은 주거 동·의료 병동·방어 구조·행정 공간이 실제로 변하며, 레벨 4 이상 조선소·해안 포대는 3단계 본체와 추가 요새 장식을 결합
...
- 정착지 첫 진입에는 핵심 1단계 건물 아틀라스만 적재하고, 저장 도시에 이미 존재하거나 건설 도감·시설 확장에서 요구된 건물군과 단계 아틀라스만 내려받습니다. 신규 정착지 기준 11개 PNG 25,834,000바이트(약 24.64MiB)와 디코딩 텍스처 약 66MiB를 초기 경로에서 유예합니다.
