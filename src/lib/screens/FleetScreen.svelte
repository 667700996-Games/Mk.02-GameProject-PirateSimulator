<script lang="ts">
  ...
  import type { FleetAssignment, FleetFormation, FleetOrderType, GameState, Officer, Ship, ZoneId } from '$lib/domain/types';
  ...
  let captains: Officer[] = $derived(game.officers.filter((officer: Officer) => officer.isCaptain));
  let fleetShips: Ship[] = $derived(game.ships.filter((ship: Ship) => !ship.isFlagship));
  let assignments: FleetAssignment[] = $derived(game.fleet.assignments);
