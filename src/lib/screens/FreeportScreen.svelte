<script lang="ts">
  ...
  function canBuy(resource: ResourceId): boolean { return resources.gold >= supplyPrice(resource) && cargoSpace(ship) >= RESOURCE_META[resource].weight * 5; }
  ...
  function buy(resource: ResourceId, amount: number): void {
    gameSession.updateGame((state) => {
      ...
      if (!canAffordGameResources(state, { gold: price })) return state;
  ...
  function recruit(): void {
    gameSession.updateGame((state) => {
      ...
      const cost = count * 32;
      if (count <= 0 || !canAffordGameResources(state, { gold: cost })) return state;
...
<section class="management-screen">
  ...
  <div class="management-grid">
    ...
    <article class="panel span-4"><span class="eyebrow">THE DROWNED CROW</span><h2>익사한 까마귀 선술집</h2><p class="muted">신참, 탈영병, 망한 항해사가 다음 배를 기다립니다.</p><button class="btn wide" onclick={recruit} disabled={recruitCount <= 0 || resources.gold < recruitCost}>선원 {recruitCount}명 모집 · {recruitCost} 금화</button><button class="btn ghost wide" style="margin-top:.5rem" onclick={drink} disabled={resources.gold < 45}>모두에게 한 잔 · 45 금화</button></article>
    <article class="panel span-4"><span class="eyebrow">BONES & DICE</span><h2>검은 뼈 도박장</h2><p class="muted">승률은 공평하지 않지만, 보상은 실제입니다.</p><button class="btn wide" onclick={gamble} disabled={resources.gold < 50}>주사위 승부 · 판돈 50</button></article>
    <article class="panel span-4"><span class="eyebrow">AUCTION HOUSE</span><h2>검은 돛 경매장</h2><p class="muted">희귀 함포 설계도가 오늘 밤 공개됩니다.</p><button class="btn wide" onclick={buyBlueprint} disabled={resources.gold < 900}>설계도 낙찰 · 900 금화</button></article>
  </div>
</section>
