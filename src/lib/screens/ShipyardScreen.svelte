...
<section class="management-screen shipbuilding-screen">
  ...
  <div class="shipyard-layout">
    <article class="panel yard-visual">
      <div class="ship-silhouette" class:building={orders.some((order) => order.state === 'BUILDING')}><div class="keel"></div><div class="ribs">{#each Array(8) as _}<i></i>{/each}</div><div class="mast"></div><div class="scaffold left"></div><div class="scaffold right"></div><div class="workers">{#each Array(Math.min(7, activeShipyard?.workers.length ?? 0)) as _, index}<i style={`--x:${12 + index * 12}%`}></i>{/each}</div></div>
    ...
    <article class="panel docked-fleet"><div class="panel-title"><div><span class="eyebrow">MOORED VESSELS</span><h2>정박 함선</h2></div></div><div class="vessel-tabs">{#each ships as ship}<button class:active={selectedShip.id === ship.id} onclick={() => (selectedShipId = ship.id)}><span>{ship.isFlagship ? '♛' : '⚓'}</span><strong>{ship.name}</strong><small>{SHIP_CLASSES[ship.class].name}</small></button>{/each}</div><div class="vessel-detail"><div><span class="eyebrow">{selectedShip.isFlagship ? 'FLAGSHIP' : 'FLEET VESSEL'}</span><h2>{selectedShip.name}</h2><p>{SHIP_CLASSES[selectedShip.class].description}</p></div><div class="plan-stats"><span><small>선체</small><b>{Math.round(selectedShip.hull)}/{selectedShip.stats.hullMax}</b></span><span><small>선원</small><b>{selectedShip.crew}/{selectedShip.stats.crewMax}</b></span><span><small>화물</small><b>{Math.round(selectedShip.cargoWeight)}/{selectedShip.stats.cargoMax}</b></span><span><small>사기</small><b>{selectedShip.morale}</b></span></div><button class="btn small" disabled={selectedShip.isFlagship} onclick={() => gameSession.updateGame((state) => ({ ...state, activeShipId: selectedShip.id, ships: state.ships.map((ship) => ({ ...ship, isFlagship: ship.id === selectedShip.id })) }), true)}>기함 지정</button></div></article>
  </div>
</section>
