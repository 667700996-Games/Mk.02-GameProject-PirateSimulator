<script lang="ts">
  ...
  import { BUILDING_LIST, BUILDINGS, SETTLEMENT_RESOURCES } from '$lib/settlement/catalog';
  import { aggregateInventory, demolishBuilding, placeBuilding, rotateBuilding, toggleBuildingPause } from '$lib/settlement/construction';
  ...
  let rightTab = $state<'selection' | 'warnings' | 'mission'>('selection');
  ...
  const overlays: { id: SettlementOverlay; name: string; icon: string }[] = [
    { id: 'none', name: '기본', icon: '◈' }, { id: 'logistics', name: '물류', icon: '⇄' }, { id: 'storage', name: '창고', icon: '▣' },
    { id: 'workers', name: '인력', icon: '♟' }, { id: 'fire', name: '화재', icon: '♨' }, { id: 'defense', name: '방어', icon: '✹' }
  ];
  ...
  function place(id: SettlementBuildingId, x: number, y: number, rotation: 0 | 1 | 2 | 3): void {
    ...
    gameSession.updateGame((state) => {
      ...
      if (!ok) return state;
      const tutorialStep = Math.max(result.state.tutorialStep, id === 'water-collector' ? 1 : id === 'lumber-camp' ? 2 : id === 'warehouse' ? 3 : id === 'fisher-hut' ? 4 : result.state.tutorialStep);
      return { ...state, settlement: { ...result.state, tutorialStep } };
    }, true);
    if (ok) {
  ...
  onMount(() => {
    ...
    void import('$lib/game/SettlementGame').then(({ createSettlementGame }) => {
      ...
      phaser = createSettlementGame(host, {
        ...
        onCancelTool: () => { buildTool = undefined; },
...
<section class="settlement-screen" data-testid="settlement-screen">
  ...
  <aside class:collapsed={!buildMenuOpen} class="build-panel panel" data-testid="build-panel">
    ...
    {#if buildMenuOpen}
      <div class="panel-title"><div><span class="eyebrow">CONSTRUCTION</span><h2>도시 건설</h2></div>{#if buildTool}<button class="btn small ghost" onclick={() => setTool()}>취소 · ESC</button>{/if}</div>
  ...
  <aside class="city-inspector panel">
    ...
    {#if rightTab === 'selection'}
      {#if selectedBuilding && selectedDefinition}
        ...
        <div class="inspector-actions"><button class="btn small" onclick={pauseSelected}>{selectedBuilding.paused ? '재개' : '일시정지'}</button><button class="btn small" onclick={rotateSelected} disabled={!['PLANNED', 'PAUSED'].includes(selectedBuilding.state)}>회전</button><button class="btn small danger-button" onclick={demolishSelected} disabled={selectedBuilding.definitionId === 'wreckage'}>철거</button></div>
      {:else}
