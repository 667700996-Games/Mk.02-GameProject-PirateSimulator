<script lang="ts">
  ...
  import {
    ...
    SETTLEMENT_RESOURCE_IDS
  } from '$lib/settlement/catalog';
  import {
    aggregateInventory,
    ...
    setBuildingRecipe,
    toggleBuildingPause
  } from '$lib/settlement/construction';
  ...
  const categories: { id: BuildingCategory; name: string; icon: string }[] = [
    ...
    { id: 'processing', name: '가공', icon: '⚒' },
    { id: 'logistics', name: '물류', icon: '⇄' },
    { id: 'housing', name: '주거', icon: '⌂' },
    { id: 'welfare', name: '복지', icon: '♨' },
    { id: 'fleet', name: '함대', icon: '⚓' },
    { id: 'military', name: '방어', icon: '♜' },
    { id: 'administration', name: '행정', icon: '♛' },
    { id: 'infrastructure', name: '지형', icon: '⌁' }
  ];
  const overlays: { id: SettlementOverlay; name: string; icon: string }[] = [
    { id: 'none', name: '기본', icon: '◈' },
    { id: 'logistics', name: '물류', icon: '⇄' },
    { id: 'traffic', name: '혼잡', icon: '⌁' },
    { id: 'food', name: '식량', icon: '◒' },
    { id: 'storage', name: '창고', icon: '▣' },
    { id: 'construction', name: '건설', icon: '⚒' },
    { id: 'needs', name: '욕구', icon: '♨' },
    { id: 'workers', name: '인력', icon: '♟' },
    { id: 'fire', name: '화재', icon: '▲' },
    { id: 'ship-supply', name: '함선', icon: '⚓' },
    { id: 'defense', name: '방어', icon: '✹' }
  ];
  ...
  let selectedDefinition = $derived(
    selectedBuilding ? BUILDINGS[selectedBuilding.definitionId] : undefined
  );
  ...
  let activeWarnings: SettlementWarning[] = $derived(
    game.settlement.warnings
      ...
      .sort(
        (a: SettlementWarning, b: SettlementWarning) =>
          severityWeight[b.severity] - severityWeight[a.severity]
      )
  );
  let activeTransports = $derived(
    game.settlement.transports.filter(
      (job: TransportJob) => !['COMPLETED', 'CANCELLED'].includes(job.state)
    ).length
  );
  let waitingTransports = $derived(
    game.settlement.transports.filter((job: TransportJob) => job.state === 'WAITING').length
  );
  let selectedInventoryEntries: [SettlementResourceId, number][] = $derived(
    selectedBuilding
      ? (
          [
            ...Object.entries(selectedBuilding.inputInventory),
            ...Object.entries(selectedBuilding.outputInventory)
          ] as [SettlementResourceId, number][]
  ...
  let statisticRows = $derived(
    SETTLEMENT_RESOURCE_IDS.map((id) => ({
      id,
      produced: game.settlement.statistics.produced[id] ?? 0,
      consumed: game.settlement.statistics.consumed[id] ?? 0,
      delivered: game.settlement.statistics.delivered[id] ?? 0
    }))
      .filter((row) => row.produced + row.consumed + row.delivered > 0)
      .sort(
        (a, b) => b.produced + b.consumed + b.delivered - (a.produced + a.consumed + a.delivered)
      )
      .slice(0, 10)
  );
  ...
  function startMoveSelected(): void {
    ...
    gameSession.addToast(
      'info',
      '시설 이동',
      '새 위치를 선택하십시오. ESC로 이동 명령을 취소합니다.'
    );
  }
  ...
  function cancelSelectedWork(): void {
    ...
    let reason = '';
    const wasUpgrade =
      selectedBuilding.state === 'UPGRADING' ||
    ...
    if (!ok) gameSession.addToast('warning', '공사 취소 불가', reason);
      ...
      gameSession.addToast(
        ...
        '공사 취소',
        wasUpgrade
          ? '확장을 중단하고 시설 가동 상태로 복구했습니다.'
          : '현장 자재를 회수하고 건설 계획을 제거했습니다.'
      );
    }
  }
  ...
  function changePriority(priority: 1 | 2 | 3 | 4 | 5): void {
    ...
    gameSession.updateGame((state) => ({
      ...state,
      settlement: setBuildingPriority(state.settlement, selectedBuilding!.id, priority)
    }));
  }
  ...
  function changeRecipe(recipeId: string): void {
    ...
    gameSession.updateGame((state) => ({
      ...state,
      settlement: setBuildingRecipe(state.settlement, selectedBuilding!.id, recipeId || undefined)
    }));
  }
  ...
  function pauseSelected(): void {
    ...
    gameSession.updateGame((state) => ({
      ...state,
      settlement: toggleBuildingPause(state.settlement, selectedBuilding.id)
    }));
  }
  ...
  onMount(() => {
    ...
    const keydown = (event: KeyboardEvent) => {
      ...
      if (event.code === 'Escape' && (buildTool || movingBuildingId)) {
      ...
      if (event.code === 'Space') {
        event.preventDefault();
        setSpeed(game.settlement.speed === 0 ? 1 : 0);
        ...
        setOverlay(game.settlement.overlay === 'logistics' ? 'none' : 'logistics');
  ...
  $effect(() => {
    ...
    soundEngine.setSettlementActivity(
      game.settlement.residents.length,
      summary.productionActive,
      game.settlement.weather,
      game.settlement.buildings.filter(
        (building: SettlementBuilding) => building.state === 'BURNING'
      ).length
    );
  });
</script>

<section class:build-open={buildMenuOpen} class="settlement-screen" data-testid="settlement-screen">
  ...
  <div class="city-status-bar panel">
    ...
    <div class="city-metric">
      <span>♟</span><b>{summary.population}</b><small>인구 / 주거 {summary.housingCapacity}</small>
    </div>
    <div class="city-metric">
      <span>◉</span><b class:danger={summary.water < summary.population}
        >{Math.floor(summary.water)}</b
      ><small>식수</small>
    </div>
    <div class="city-metric">
      <span>◒</span><b class:danger={summary.foodDays < 2}>{summary.foodDays.toFixed(1)}일</b><small
        >식량 잔여</small
      >
    </div>
    <div class="city-metric">
      <span>♨</span><b>{Math.round(summary.morale)}%</b><small>사기</small>
    </div>
    <div class="city-metric">
      <span>⇄</span><b class:danger={waitingTransports > 3}>{activeTransports}</b><small
        >운송 중 / 대기 {waitingTransports}</small
      >
    </div>
    <div class="city-metric"><span>✹</span><b>{summary.defense}</b><small>해안 방어</small></div>
    <div class="city-metric">
      <span>⚑</span><b class:danger={game.settlement.threat.active}
        >{game.settlement.threat.active
          ? `${game.settlement.threat.etaHours.toFixed(1)}h`
          : '안전'}</b
      ><small>해군 위협</small>
    </div>
    <div class="speed-control" aria-label="게임 속도">
      <button
        class:active={game.settlement.speed === 0}
        onclick={() => setSpeed(0)}
        title="일시정지">Ⅱ</button
      >
      {#each [1, 2, 3] as speed}<button
          class:active={game.settlement.speed === speed}
          onclick={() => setSpeed(speed as 1 | 2 | 3)}>{speed}×</button
        >{/each}
    </div>
  </div>
  ...
  <aside class:collapsed={!buildMenuOpen} class="build-panel panel" data-testid="build-panel">
    <button
      ...
      aria-label={buildMenuOpen ? '건설 메뉴 닫기' : '건설 메뉴 열기'}
      aria-expanded={buildMenuOpen}
      onclick={() => (buildMenuOpen = !buildMenuOpen)}>{buildMenuOpen ? '‹' : 'B'}</button
    >
    {#if buildMenuOpen}
      <div class="panel-title">
        ...
        {#if buildTool || movingBuildingId}<button class="btn small ghost" onclick={() => setTool()}
            >취소 · ESC</button
          >{/if}
      </div>
      <div class="category-tabs">
        {#each categories as category}<button
            ...
            title={category.name}><span>{category.icon}</span><small>{category.name}</small></button
          >{/each}
      </div>
      <div class="building-list">
        {#each categoryBuildings as definition}
          {@const locked =
            !!definition.unlock &&
            !game.settlement.progression.unlocked.includes(definition.unlock)}
          <button
            class:selected={buildTool === definition.id}
            class:locked
            class="building-option"
            onclick={() => !locked && startPlacement(definition.id)}
            disabled={locked}
            data-testid={`build-${definition.id}`}
          >
      ...
      <div class="build-help">
        <kbd>클릭</kbd> 배치 · <kbd>R</kbd> 회전 · <kbd>드래그</kbd> 카메라 · <kbd>휠</kbd> 확대
      </div>
    {/if}
  </aside>
  ...
  <aside class="city-inspector panel">
    <div class="inspector-tabs">
      <button class:active={rightTab === 'selection'} onclick={() => (rightTab = 'selection')}
        >선택</button
      >
      <button class:active={rightTab === 'warnings'} onclick={() => (rightTab = 'warnings')}
        >경고 <span class:urgent={activeWarnings.length > 0}>{activeWarnings.length}</span></button
      >
      <button class:active={rightTab === 'statistics'} onclick={() => (rightTab = 'statistics')}
        >통계</button
      >
      <button class:active={rightTab === 'mission'} onclick={() => (rightTab = 'mission')}
        >표류 일지</button
      >
    </div>
    {#if rightTab === 'selection'}
      {#if selectedBuilding && selectedDefinition}
        <div class="selection-head">
          <div class="selection-icon">{selectedDefinition.icon}</div>
          <div>
            <span class="eyebrow">{selectedDefinition.category}</span>
            <h2>{selectedDefinition.name}</h2>
            <small class="muted"
              >구역 {selectedBuilding.x}:{selectedBuilding.y} · 높이 {game.settlement.island.tiles[
                selectedBuilding.y * game.settlement.island.width + selectedBuilding.x
              ]?.elevation}</small
            >
          </div>
        </div>
        <div class="state-banner" class:blocked={selectedBuilding.state === 'BLOCKED'}>
          <b>{selectedBuilding.state}</b><span
            >{selectedBuilding.statusReason ?? '정상 운영 중'}</span
          >
        </div>
        {#if selectedBuilding.state !== 'ACTIVE'}<div class="meter large">
            <span
              style={`--value:${selectedBuilding.constructionProgress * 100}%;--meter-color:#c49b53`}
            ></span>
          </div>{/if}
        <div class="inspector-grid">
          <div>
            <small>작업자</small><b
              >{selectedBuilding.workers.length} / {selectedDefinition.workerSlots}</b
            >
          </div>
        ...
        <div class="priority-control">
          ...
          <div>
            {#each [1, 2, 3, 4, 5] as priority}<button
                class:active={selectedBuilding.constructionPriority === priority}
                onclick={() => changePriority(priority as 1 | 2 | 3 | 4 | 5)}>{priority}</button
              >{/each}
          </div>
        </div>
        {#if selectedDefinition.recipes.length > 0}
          <label class="recipe-select"
            >생산법<select
              value={selectedBuilding.recipeId ?? ''}
              onchange={(event) => changeRecipe(event.currentTarget.value)}
              ><option value="">생산 중단</option
              >{#each selectedDefinition.recipes as recipeId}<option value={recipeId}
                  >{RECIPES[recipeId]?.name}</option
                >{/each}</select
            ></label
          >
        {/if}
        {#if ['lumber-camp', 'quarry', 'iron-mine', 'copper-mine'].includes(selectedBuilding.definitionId)}<p
            class="deposit-readout"
          >
            천연자원 잔량 <b
              >{Math.floor(
                game.settlement.island.tiles[
                  selectedBuilding.y * game.settlement.island.width + selectedBuilding.x
                ]?.resourceRemaining ?? 0
              )}</b
            >
        ...
        <div class="inventory-grid">
          {#each selectedInventoryEntries as [id, value]}
            <span
              ><i>{SETTLEMENT_RESOURCES[id].icon}</i>{SETTLEMENT_RESOURCES[id].name}<b
                >{Math.floor(value * 10) / 10}</b
              ></span
            >
          {:else}<p class="faint">운송되거나 생산된 물자가 없습니다.</p>{/each}
        </div>
        {#if selectedBuilding.state === 'ACTIVE' && selectedBuilding.level < buildingMaxLevel(selectedBuilding.definitionId)}<div
            class="upgrade-cost"
          >
        ...
        <div class="inspector-actions">
          <button class="btn small" onclick={pauseSelected}
            >{selectedBuilding.paused ? '재개' : '일시정지'}</button
          ><button
            class="btn small"
            onclick={rotateSelected}
            disabled={!['PLANNED', 'PAUSED'].includes(selectedBuilding.state)}>회전</button
          ><button
            class="btn small"
            onclick={startMoveSelected}
            disabled={!['PLANNED', 'PAUSED'].includes(selectedBuilding.state)}>이동</button
          ><button
            class="btn small"
            onclick={upgradeSelected}
            disabled={selectedBuilding.state !== 'ACTIVE' ||
              selectedBuilding.level >= buildingMaxLevel(selectedBuilding.definitionId)}
            >확장</button
      ...
      {:else}
        <div class="empty-selection">
          <span>✥</span>
          <h2>섬을 살펴보십시오</h2>
          <p>건물을 선택하면 작업자, 현장 재고, 생산 중단 원인과 명령이 표시됩니다.</p>
          <div class="legend">
    ...
    {:else if rightTab === 'warnings'}
      <div class="panel-title">
        <div>
          <span class="eyebrow">WATCH REPORT</span>
          <h2>정착지 경고</h2>
        </div>
      </div>
      <div class="warning-list">
        {#each activeWarnings as warning}<button
            class:emergency={warning.severity === 'emergency'}
            class:danger-warning={warning.severity === 'danger'}
            onclick={() => focusWarning(warning.buildingId)}
            ><span
              >{warning.severity === 'emergency'
                ...
                : warning.severity === 'danger'
                  ...
                  : '◆'}</span
            >
            <div><strong>{warning.title}</strong><small>{warning.detail}</small></div></button
          >{:else}<div class="all-clear">
            <span>✓</span><strong>중대한 경고 없음</strong><small
              >망보는 자들이 섬과 수평선을 감시하고 있습니다.</small
            >
          </div>{/each}
      </div>
    {:else if rightTab === 'statistics'}
      <div class="panel-title">
        <div>
          <span class="eyebrow">FLOW LEDGER</span>
          <h2>생산·물류 통계</h2>
        </div>
      </div>
      <div class="stat-summary">
        <span><small>완공</small><b>{game.settlement.statistics.completedBuildings}</b></span><span
          ><small>활성 운송</small><b>{activeTransports}</b></span
        ><span><small>생산 병목</small><b>{summary.productionBlocked}</b></span>
      </div>
      <div class="flow-table">
        <header><span>상품</span><b>생산/h</b><b>소비/h</b><b>운송/h</b></header>
        {#each statisticRows as row}<div>
            <span>{SETTLEMENT_RESOURCES[row.id].icon} {SETTLEMENT_RESOURCES[row.id].name}</span><b
              >{(row.produced / elapsedHours).toFixed(1)}</b
            ><b>{(row.consumed / elapsedHours).toFixed(1)}</b><b
              >{(row.delivered / elapsedHours).toFixed(1)}</b
            >
          </div>{:else}<p class="faint">
            첫 생산 또는 운송이 끝나면 흐름 기록이 표시됩니다.
          </p>{/each}
      </div>
      <p class="analysis-note">
        경고에서 병목 시설을 선택하고, 물류·창고·인력 오버레이를 겹쳐 원인을 추적하십시오.
      </p>
    {:else}
  ...
  <div class="overlay-toolbar panel">
    ...
    {#each overlays as overlay}<button
        ...
        title={`${overlay.name} 오버레이`}><span>{overlay.icon}</span>{overlay.name}</button
      >{/each}
    <i></i>
...
<style>
  ...
  .city-vignette {
    ...
    pointer-events: none;
    z-index: 2;
    box-shadow: inset 0 0 150px 28px #010a0dbd;
    background: linear-gradient(180deg, #03111544, transparent 18% 76%, #020a0ebd);
  }
  .city-status-bar {
    position: absolute;
    z-index: 10;
    ...
    align-items: stretch;
    padding: 0;
    background: linear-gradient(90deg, #07191dec, #0d2528e8 55%, #071619ef);
  ...
  .city-metric {
    ...
    column-gap: 0.48rem;
    align-items: end;
    padding: 0.68rem clamp(0.55rem, 1.2vw, 1rem);
  ...
  .building-list {
    ...
    padding: 0.65rem 0.15rem 0.4rem 0;
  }
  .building-option {
    position: relative;
    display: grid;
    ...
    text-align: left;
    border: 1px solid var(--line-soft);
    background: linear-gradient(135deg, #173034aa, #07171bd9);
    color: var(--ink);
    padding: 0.58rem;
    cursor: pointer;
  ...
  .building-option.selected {
    ...
    background: linear-gradient(135deg, #4b3d29c4, #0d2527e8);
  }
  ...
  .warning-list button {
    ...
    gap: 0.5rem;
    text-align: left;
    border: 1px solid #73553a;
    background: #2c241a;
    color: var(--ink);
    padding: 0.6rem;
    cursor: pointer;
  }
  .warning-list button.danger-warning,
  .warning-list button.emergency {
    border-color: #a94e42;
    background: #3a1d1b;
  }
  .warning-list button > span {
    font-size: 1rem;
    color: #e78b68;
  }
  .warning-list strong,
  ...
  .objective-progress span {
    ...
    width: var(--progress);
    height: 100%;
    background: linear-gradient(90deg, #81502d, #d3aa5b);
  }
  .overlay-toolbar {
    position: absolute;
    z-index: 12;
  ...
  .overlay-toolbar button:hover,
  .overlay-toolbar button.active {
    color: var(--ink);
    border-color: var(--line);
    background: #203431;
  }
