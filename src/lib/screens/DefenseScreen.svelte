...
<section class="event-screen defense-screen">
  <article class="event-card panel defense-card">
    <header class="defense-header">
      ...
      <div class="alarm-clock"><span class="tag danger">{attacker.name}</span>{#if ['warning', 'preparation'].includes(game.defense.stage)}<strong>{Math.ceil(game.defense.timeToAttack)}초</strong><small>포격 개시까지</small>{/if}</div>
    </header>
    ...
    {:else}
      <div class="result-banner" class:victory={game.defense.outcome === 'victory'}><span class="eyebrow">AFTER ACTION REPORT</span><h2>{game.defense.outcome === 'victory' ? '검은물결이 침공자를 삼켰습니다' : '본거지는 살아남았지만 대가를 치렀습니다'}</h2><p>{game.defense.damage?.join(' · ')}</p>{#if game.defense.losses}<p>부상 {game.defense.losses.wounded} · 사망·이탈 {game.defense.losses.killed} · 함선 손실 {game.defense.losses.shipsLost}</p>{/if}{#if Object.keys(game.defense.reward ?? {}).length}<p class="brass">전리품: {Object.entries(game.defense.reward ?? {}).map(([id, amount]) => `${id} ${amount}`).join(' · ')}</p>{/if}<button class="btn primary" onclick={() => update(claimDefenseResult, true)}>손상 점검 후 본거지로</button></div>
    {/if}
