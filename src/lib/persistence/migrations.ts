...
function assertUnique(values: string[], label: string): void {
  if (values.some((id) => typeof id !== 'string' || id.length === 0) || new Set(values).size !== values.length) {
    throw new SaveMigrationError(`${label} 식별자가 중복되거나 손상되었습니다.`);
  }
}

function assertFinite(value: number, label: string, minimum = 0, maximum = Number.MAX_SAFE_INTEGER): void {
  if (!Number.isFinite(value) || value < minimum || value > maximum) throw new SaveMigrationError(`${label} 수치가 손상되었습니다.`);
}
...
function validateInventory(inventory: unknown, label: string): void {
  if (!inventory || typeof inventory !== 'object' || Array.isArray(inventory)) throw new SaveMigrationError(`${label} 재고가 손상되었습니다.`);
  for (const [id, quantity] of Object.entries(inventory)) {
    if (!settlementResourceIds.has(id) || typeof quantity !== 'number') throw new SaveMigrationError(`${label}에 알 수 없는 자원이 있습니다.`);
    assertFinite(quantity, `${label} ${id}`, 0, 1_000_000_000);
  }
}

function validateIdentifiers(state: GameState): void {
  assertUnique(state.ships.map((ship) => ship.id), '함선');
  assertUnique(state.officers.map((officer) => officer.id), '장교');
  assertUnique(state.settlement.residents.map((resident) => resident.id), '주민');
  assertUnique(state.settlement.buildings.map((building) => building.id), '건물');
  assertUnique(state.settlement.transports.map((transport) => transport.id), '운송');
  assertUnique(state.settlement.expeditions.map((expedition) => expedition.id), '원정');
  if (!state.ships.some((ship) => ship.id === state.activeShipId)) throw new SaveMigrationError('활성 함선 참조가 손상되었습니다.');
}

function validateSettlement(state: GameState): void {
  const settlement = state.settlement;
  if (settlement.residents.length > 5_000 || settlement.buildings.length > 5_000 || settlement.transports.length > 20_000 || settlement.expeditions.length > 200) {
  ...
  if (!Number.isInteger(island.width) || !Number.isInteger(island.height) || island.tiles.length !== island.width * island.height) throw new SaveMigrationError('섬 지도 크기가 손상되었습니다.');
  for (const tile of island.tiles) {
    ...
    if (tile.resourceRemaining !== undefined) assertFinite(tile.resourceRemaining, '천연자원 잔량', 0, 1_000_000);
  }
  ...
  for (const resident of settlement.residents) {
    for (const [label, number] of Object.entries({ health: resident.health, morale: resident.morale, loyalty: resident.loyalty, fatigue: resident.fatigue, experience: resident.experience })) assertFinite(number, `주민 ${label}`, 0, 1_000_000);
    for (const [need, score] of Object.entries(resident.needs)) assertFinite(score, `주민 욕구 ${need}`, 0, 100);
    if (resident.homeId && !buildingIds.has(resident.homeId)) throw new SaveMigrationError('주민 주거 참조가 손상되었습니다.');
    if (resident.workplaceId && !buildingIds.has(resident.workplaceId)) throw new SaveMigrationError('주민 근무지 참조가 손상되었습니다.');
  ...
  for (const building of settlement.buildings) {
    if (!BUILDINGS[building.definitionId]) throw new SaveMigrationError('알 수 없는 건물 정의가 저장되어 있습니다.');
    ...
    if (building.recipeId && (!RECIPES[building.recipeId] || !BUILDINGS[building.definitionId]?.recipes.includes(building.recipeId))) throw new SaveMigrationError('건물 생산법 참조가 손상되었습니다.');
    if (building.workers.some((id) => !residentIds.has(id))) throw new SaveMigrationError('건물 작업자 참조가 손상되었습니다.');
    ...
    for (const [id, reserved] of Object.entries(building.reservedInventory)) {
      if ((reserved ?? 0) > (building.outputInventory[id as keyof typeof building.outputInventory] ?? 0) + .001) throw new SaveMigrationError('예약 재고가 실제 출력 재고를 초과했습니다.');
    }
  }
  for (const transport of settlement.transports) {
    if (!buildingIds.has(transport.sourceBuildingId) || !buildingIds.has(transport.targetBuildingId)) throw new SaveMigrationError('운송 건물 참조가 손상되었습니다.');
    if (transport.haulerId && !residentIds.has(transport.haulerId)) throw new SaveMigrationError('운송 인력 참조가 손상되었습니다.');
    if (!settlementResourceIds.has(transport.resourceId)) throw new SaveMigrationError('운송 자원 참조가 손상되었습니다.');
    assertFinite(transport.amount, '운송 수량', .0001, 1_000_000);
    assertFinite(transport.progress, '운송 진행도', 0, 1);
  }
  for (const expedition of settlement.expeditions) {
    if (expedition.shipIds.some((id) => !shipIds.has(id)) || expedition.captainIds.some((id) => !officerIds.has(id)) || expedition.crewIds.some((id) => !residentIds.has(id))) throw new SaveMigrationError('원정 편성 참조가 손상되었습니다.');
...
function validateGameNumbers(state: GameState): void {
  ...
  for (const [id, quantity] of Object.entries(state.resources)) assertFinite(quantity, `게임 자원 ${id}`, 0, 1_000_000_000);
  ...
  assertFinite(state.settlement.residentUpdateCursor, '주민 갱신 커서', 0, Math.max(0, state.settlement.residents.length));
}
