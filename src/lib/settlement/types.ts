...
export type SettlementResourceCategory = 'basic' | 'processed' | 'living' | 'military' | 'luxury' | 'loot';

export type SettlementResourceId =
  | 'gold' | 'logs' | 'stone' | 'iron-ore' | 'copper-ore' | 'fiber' | 'water' | 'fish' | 'fruit' | 'grain' | 'sulfur' | 'raw-meat'
  | 'planks' | 'stone-blocks' | 'iron-ingots' | 'copper-ingots' | 'rope' | 'cloth' | 'leather' | 'charcoal' | 'powder' | 'glass' | 'flour'
  | 'hardtack' | 'fish-stew' | 'meat-dish' | 'rum' | 'beer' | 'clothes' | 'boots' | 'hats' | 'medicine' | 'tobacco'
  | 'cannonballs' | 'powder-kegs' | 'pistols' | 'muskets' | 'cutlasses' | 'armor' | 'navigation-tools' | 'telescopes' | 'sails' | 'ship-parts' | 'cannons' | 'tools'
  | 'jeweled-ornaments' | 'fine-clothes' | 'aged-rum' | 'precision-instruments' | 'prosthetics' | 'officer-pistols' | 'rare-medicine' | 'figureheads'
  | 'royal-coins' | 'military-maps' | 'naval-ciphers' | 'ancient-relics' | 'rare-blueprints' | 'legendary-parts' | 'monster-materials'
  | 'silver' | 'spices' | 'wine' | 'whale-oil' | 'rare-metal' | 'royal-equipment' | 'foreign-textiles';
...
export type TerrainType = 'deep-water' | 'reef' | 'beach' | 'coast' | 'plain' | 'forest' | 'slope' | 'cliff' | 'highland' | 'cave' | 'ravine' | 'wetland' | 'stone-deposit' | 'iron-vein' | 'copper-vein';
...
export type BuildingCategory = 'gathering' | 'processing' | 'logistics' | 'housing' | 'welfare' | 'fleet' | 'military' | 'administration' | 'infrastructure';
export type BuildingState = 'PLANNED' | 'CONSTRUCTING' | 'ACTIVE' | 'PAUSED' | 'BLOCKED' | 'DAMAGED' | 'BURNING' | 'DESTROYED' | 'UPGRADING';

export type SettlementBuildingId =
  | 'wreckage' | 'campfire' | 'tent' | 'water-collector' | 'fisher-hut' | 'lumber-camp' | 'quarry' | 'iron-mine' | 'copper-mine' | 'farm' | 'hunter-hut'
  | 'sawmill' | 'smelter' | 'forge' | 'mill' | 'bakery' | 'cookhouse' | 'distillery' | 'weaver' | 'powder-workshop' | 'ammunition-workshop'
  | 'warehouse' | 'local-storage' | 'distribution-depot' | 'dock-warehouse' | 'cargo-lift' | 'zipline-post' | 'bridge' | 'stairs' | 'ramp' | 'cliff-platform'
  | 'bunkhouse' | 'barracks' | 'skilled-house' | 'officer-quarters'
  | 'tavern' | 'gambling-den' | 'arena' | 'infirmary' | 'bathhouse' | 'festival-square'
  | 'small-dock' | 'shipyard' | 'dry-dock' | 'supply-depot' | 'cannon-foundry' | 'training-yard'
  | 'watchtower' | 'coastal-battery' | 'fort-wall' | 'powder-magazine' | 'guard-post' | 'signal-tower'
  | 'captains-lodge' | 'pirate-council' | 'intelligence-network' | 'bounty-board' | 'expedition-office';
...
export type TransportState = 'WAITING' | 'ASSIGNED' | 'PICKING_UP' | 'DELIVERING' | 'COMPLETED' | 'CANCELLED';
...
export type SettlementOverlay = 'none' | 'logistics' | 'food' | 'construction' | 'military' | 'consumer' | 'ship-supply' | 'storage' | 'traffic' | 'needs' | 'workers' | 'fire' | 'defense' | 'housing';
