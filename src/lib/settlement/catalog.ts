...
export const RECIPES: Record<string, RecipeDefinition> = Object.fromEntries(([
  ...
  { id: 'grow-fruit', name: '과수 재배', buildingType: 'farm', inputs: { water: 1 }, outputs: { fruit: 4 }, durationMinutes: 22, workers: { farmer: 2 }, danger: 1 },
  ...
  { id: 'smelt-glass', name: '유리 제련', buildingType: 'smelter', inputs: { stone: 2, charcoal: 1 }, outputs: { glass: 2 }, durationMinutes: 20, workers: { smelter: 2 }, danger: 9 },
  ...
  { id: 'forge-navigation-tools', name: '항해 도구 제작', buildingType: 'forge', inputs: { 'copper-ingots': 2, planks: 1 }, outputs: { 'navigation-tools': 1 }, durationMinutes: 28, workers: { blacksmith: 2 }, danger: 5 },
  { id: 'forge-firearms', name: '권총과 장총 제작', buildingType: 'forge', inputs: { 'iron-ingots': 4, planks: 2 }, outputs: { pistols: 1, muskets: 1 }, durationMinutes: 38, workers: { blacksmith: 3 }, danger: 10 },
  { id: 'forge-armor', name: '해적 갑옷 제작', buildingType: 'forge', inputs: { 'iron-ingots': 3, leather: 2 }, outputs: { armor: 1 }, durationMinutes: 32, workers: { blacksmith: 2 }, danger: 7 },
  { id: 'precision-navigation', name: '정밀 항해 장비', buildingType: 'forge', inputs: { 'copper-ingots': 3, glass: 2, 'navigation-tools': 1 }, outputs: { 'precision-instruments': 1 }, durationMinutes: 48, workers: { blacksmith: 3 }, danger: 5 },
  ...
  { id: 'prepare-medicine', name: '기초 약품 조제', buildingType: 'cookhouse', inputs: { fruit: 2, cloth: 1 }, outputs: { medicine: 1 }, durationMinutes: 24, workers: { cook: 2 }, danger: 2 },
  ...
  { id: 'sew-boots', name: '작업 장화', buildingType: 'weaver', inputs: { leather: 2, cloth: 1 }, outputs: { boots: 2 }, durationMinutes: 22, workers: { tailor: 2 }, danger: 2 },
  { id: 'sew-hats', name: '해적 모자', buildingType: 'weaver', inputs: { cloth: 2, leather: 1 }, outputs: { hats: 2 }, durationMinutes: 20, workers: { tailor: 2 }, danger: 1 },
  ...
  { id: 'pack-powder-kegs', name: '화약통 포장', buildingType: 'powder-workshop', inputs: { powder: 3, cloth: 1 }, outputs: { 'powder-kegs': 1 }, durationMinutes: 18, workers: { 'powder-maker': 2 }, danger: 28 },
...
const buildingRows: BuildingDefinition[] = [
  ...
  { id: 'farm', name: '계단식 농장', category: 'gathering', icon: '⋮', footprint: [3, 2], terrainRules: ['plain'], maxElevation: 2, constructionCost: { logs: 8, tools: 2 }, constructionMinutes: 24, workerJob: 'farmer', workerSlots: 5, storage: 36, recipes: ['grow-grain', 'grow-fruit'], description: '비옥한 평지에서 곡물과 과일을 재배한다.' },
  ...
  { id: 'smelter', name: '제련소', category: 'processing', icon: '♨', footprint: [2, 2], terrainRules: FLAT, constructionCost: { planks: 10, stone: 12, tools: 3 }, constructionMinutes: 42, workerJob: 'smelter', workerSlots: 5, storage: 36, recipes: ['burn-charcoal', 'smelt-iron', 'smelt-copper', 'smelt-glass'], unlock: 'prosperity-foundry', description: '광석, 석재와 숯을 사용해 금속·유리를 제련한다.' },
  { id: 'forge', name: '대장간', category: 'processing', icon: '⚒', footprint: [2, 2], terrainRules: FLAT, constructionCost: { planks: 12, 'stone-blocks': 8, 'iron-ingots': 4 }, constructionMinutes: 46, workerJob: 'blacksmith', workerSlots: 5, storage: 36, recipes: ['forge-tools', 'forge-cutlasses', 'forge-navigation-tools', 'forge-firearms', 'forge-armor', 'precision-navigation'], unlock: 'prosperity-foundry', description: '도구, 무기와 항해 장비를 제작한다.' },
  ...
  { id: 'cookhouse', name: '공동 조리소', category: 'processing', icon: '♨', footprint: [2, 1], terrainRules: FLAT, constructionCost: { logs: 8, stone: 6 }, constructionMinutes: 22, workerJob: 'cook', workerSlots: 3, storage: 30, recipes: ['cook-fish-stew', 'prepare-medicine'], description: '정착민에게 따뜻한 식사와 기초 약품을 공급한다.' },
  ...
  { id: 'weaver', name: '직조소', category: 'processing', icon: '▧', footprint: [2, 2], terrainRules: FLAT, constructionCost: { planks: 12, rope: 4 }, constructionMinutes: 34, workerJob: 'tailor', workerSlots: 4, storage: 36, recipes: ['weave-cloth', 'twist-rope', 'sew-clothes', 'sew-boots', 'sew-hats', 'make-sails'], description: '섬유와 가죽으로 밧줄, 의복과 돛을 제작한다.' },
  { id: 'powder-workshop', name: '화약 공방', category: 'processing', icon: '✦', footprint: [2, 2], terrainRules: FLAT, constructionCost: { planks: 10, 'stone-blocks': 10, tools: 4 }, constructionMinutes: 48, workerJob: 'powder-maker', workerSlots: 4, storage: 24, recipes: ['mix-powder', 'pack-powder-kegs'], range: 3, unlock: 'infamy-black-powder', description: '위험한 화약과 화약통을 제조한다. 주거지와 거리를 두어야 한다.' },
